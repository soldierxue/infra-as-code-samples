...
resource "aws_iam_instance_profile" "ecs_instance" {
  name = "${format("ecs-profile-%s-%s", var.name, var.environment)}"
...
resource "aws_iam_role" "ecs_instance" {
  name = "${format("role-ecs-%s-%s", var.name, var.environment)}"
...
resource "aws_iam_role_policy" "ecs_instance_policy" {
  name = "${format("policy-ecs-%s-%s", var.name, var.environment)}"
...
resource "aws_iam_role" "ecs_service" {
  name = "${format("role-ecs-srv-%s-%s", var.name, var.environment)}"
...
resource "aws_iam_role" "ecs_service_autoscale" {
  name = "${format("role-ecs-asg-%s-%s", var.name, var.environment)}"
